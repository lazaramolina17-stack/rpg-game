...
var Renderer = class {
  ...
  drawDndCharacterSheet(ctx, w, h) {
    ...
    ctx.textAlign = "center";
    ctx.textBaseline = "middle";
    ctx.fillText("\xD7", cbx + cbs / 2, cby + cbs / 2);
    ctx.textBaseline = "alphabetic";
    ...
    ctx.clip();
    let curY = baseY;
    const fillRoundRect = (x, y, w2, h2, r) => {
      ctx.beginPath();
      ctx.moveTo(x + r, y);
      ctx.lineTo(x + w2 - r, y);
    ...
    const secH = (label, color, content) => {
      const gap = 6;
      curY += gap;
    ...
    const drawHBar = (label, val, max, x, y, bw, color) => {
      ctx.fillStyle = "#64748b";
    ...
    const attrKeys = ["strength", "dexterity", "constitution", "intelligence", "wisdom", "charisma"];
